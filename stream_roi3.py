...
class ThermalCamera:
    def __init__(self, roi=(0, 0, 61, 61), com_port=None):        #camera connection, image processing, and video streaming
        """
    ...
    def draw_grid(self, frame):
        ...
        #overlay = frame.copy()  # Create an overlay to reduce opacity ----------------------------
    ...
    def calculate_temperatures(self, frame, x1, y1, x2, y2):
        ...
        self.temps = {name: round(float(np.mean(region)), 2) for name, region in sections.items()}
        return self.temps

    async def send_temperature(self, websocket, path):
        """Continuously sends the latest average temperature values to clients."""
        while self.running:
            await websocket.send(json.dumps(self.temps))
            await asyncio.sleep(1)  # Send data every 1 second

    def start_websocket_server(self):
        """Starts a WebSocket server for sending temperature data."""
        loop = asyncio.new_event_loop()
        asyncio.set_event_loop(loop)
        server = websockets.serve(self.send_temperature, "0.0.0.0", 8765)
...
if __name__ == "__main__":
    roi = (10, 1, 70, 61)                          # Large square ROI change for the camera fov crop
